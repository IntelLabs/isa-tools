...
def get_c_flags(iii, backend):
    if "opam" in iii:
        ...
        path = os.path.join(rootdir, "lib/iii/runtime_include")
        c_flags = [f"-I{path}"]
...
def get_ld_flags(iii, backend):
    if "opam" in iii:
        ...
        bindir = os.path.dirname(iii)
        rootdir = os.path.dirname(bindir)
        path = os.path.join(rootdir, "lib/iii/runtime/libASL.a")
        ld_flags = [path]
    if backend == "sc":
        sc_types_dir = os.environ.get('SC_TYPES_DIR')
        if not sc_types_dir:
...
def main() -> int:
    ...
    parser.add_argument("--constraint-checks", help="perform type constraint checks", action=argparse.BooleanOptionalAction, default=False)
